...
type AuthUserRole = 'ADMIN' | 'OPERATOR' | 'VIEWER'
...
const authUserFromReq = (req: Request) => (req as any).authUser as AuthUser | undefined
...
function normalizeHlsUrlForLocalGateway(hlsUrl?: string) {
  ...
  try {
    ...
    if (!hostIsLocal || u.port !== '8888') return trimmed
...
function normalizeWhepUrlForLocalGateway(whepUrl?: string) {
  ...
  try {
    const u = new URL(trimmed)
    const hostIsLocal = u.hostname === 'localhost' || u.hostname === '127.0.0.1'
    if (!hostIsLocal || u.port !== '8889') return trimmed
...
async function tcpProbe(host: string, port: number, timeoutMs: number) {
  ...
  return await new Promise<{ ok: true; ms: number } | { ok: false; ms: number; error: string }>((resolve) => {
    const socket = new net.Socket()
    const done = (result: { ok: true; ms: number } | { ok: false; ms: number; error: string }) => {
      try {
...
async function rtspOptionsProbe(rtspUrl: string, timeoutMs: number) {
  const startedAt = Date.now()
  ...
  try {
    ...
    return { ok: false as const, ms: 0, error: 'Invalid RTSP URL' }
  }
  ...
  return await new Promise<{ ok: true; ms: number } | { ok: false; ms: number; error: string }>((resolve) => {
    ...
    socket.on('data', (chunk) => {
      ...
      if (!m) return done({ ok: false, ms: Date.now() - startedAt, error: 'No RTSP response' })

      const code = Number(m[2])
      const text = (m[3] ?? '').trim()

      if (code === 200 || code === 401) return done({ ok: true, ms: Date.now() - startedAt })
...
async function fetchProbe(url: string, timeoutMs: number) {
  const startedAt = Date.now()
  ...
  try {
    ...
    return { ok: false, status: undefined as number | undefined, ms: Date.now() - startedAt, error: msg }
  } finally {
    clearTimeout(t)
  }
}

function isLocalMtxHls(url: string) {
  try {
    const u = new URL(url)
    const hostIsLocal = u.hostname === 'localhost' || u.hostname === '127.0.0.1'
...
async function dbRows<T = any>(db: Pool, sql: string, params: unknown[] = []) {
  const [rows] = await db.execute(sql, params)
  return rows as T[]
}

async function loadAccessLocationIds(db: Pool, userId: string) {
  const rows = await dbRows<any>(db, 'SELECT locationId FROM user_location_access WHERE userId = ? AND canView = 1', [userId])
  return rows.map((row) => String(row.locationId))
}

function requireUser(db: Pool) {
  ...
  return async (req: Request, res: Response, next: () => void) => {
    ...
    try {
      const rows = await dbRows<any>(db, 'SELECT id, name, email, role FROM users WHERE id = ? LIMIT 1', [userId])
      const user = rows[0]
...
async function logAudit(db: Pool, payload: { actor: string; action: string; entityId?: string; entityName?: string; details?: Record<string, unknown> }) {
  await db.execute(
    'INSERT INTO audit_events (id, actor, action, entityId, entityName, details, at) VALUES (UUID(), ?, ?, ?, ?, ?, NOW())',
    [payload.actor, payload.action, payload.entityId ?? null, payload.entityName ?? null, payload.details ? JSON.stringify(payload.details) : null],
  )
}
...
export function buildRoutes(db: Pool) {
  ...
  router.post('/auth/login', async (req, res) => {
    ...
    const rows = await dbRows<any>(db, 'SELECT * FROM users WHERE LOWER(email) = ? LIMIT 1', [email])
    const user = rows[0]
    if (!user || String(user.password_hash ?? '') !== password) {
      return res.status(401).json({ error: 'Invalid credentials' })
    }

    const accessRows = await dbRows<any>(
      db,
      'SELECT locationId, canView, canControl FROM user_location_access WHERE userId = ? ORDER BY locationId',
      [user.id],
    )
  ...
  router.get('/admin/users', async (req, res) => {
    ...
    const rows = await dbRows<any>(db, 'SELECT id, name, email, role, createdAt FROM users ORDER BY createdAt DESC')
    res.json(
      rows.map((row) => ({
        id: String(row.id),
  ...
  router.post('/admin/users', async (req, res) => {
    ...
    const id = randomUUID()
    await db.execute(
      'INSERT INTO users (id, name, email, password_hash, role, createdAt) VALUES (?, ?, ?, ?, ?, NOW())',
  ...
  router.put('/admin/users/:id', async (req, res) => {
    ...
    if (req.body?.password != null && String(req.body.password).trim()) patch.password_hash = String(req.body.password).trim()
    if (req.body?.role != null) patch.role = String(req.body.role).toUpperCase()
    ...
    const rows = await dbRows<any>(db, 'SELECT id, name, email, role, createdAt FROM users WHERE id = ? LIMIT 1', [id])
    const user = rows[0]
  ...
  router.put('/admin/users/:id/access', async (req, res) => {
    ...
    try {
      ...
      for (const row of access) {
        ...
        await conn.execute(
          'INSERT INTO user_location_access (userId, locationId, canView, canControl, createdAt) VALUES (?, ?, ?, ?, NOW())',
          [
            req.params.id,
  ...
  router.get('/debug/cameras/:id/stream-check', async (req, res) => {
    ...
    const cams = await dbRows<any>(
      ...
      user?.role === 'ADMIN'
        ...
        : `SELECT c.*
           ...
           WHERE c.id = ? LIMIT 1`,
      user?.role === 'ADMIN' ? [req.params.id] : [user?.id ?? '', req.params.id],
    )
    const cam = cams[0]
    if (!cam) return res.status(404).json({ error: 'Camera not found' })

    let rtspHost: string | undefined
    let rtspPort = 554

    try {
      const u = new URL(String(cam.rtspUrl))
      rtspHost = u.hostname
      if (u.port) rtspPort = Number(u.port)
    } catch {
    ...
    const rtsp = rtspHost
      ...
      : ({ ok: false, ms: 0, error: 'Missing RTSP host' } as const)
    ...
    const hls = hlsUrl ? await fetchProbe(hlsUrl, hlsTimeoutMs) : ({ ok: false, ms: 0, error: 'No HLS URL configured' } as const)
  ...
  router.get('/health/settings', async (_req, res) => {
    const rows = await dbRows<any>(db, 'SELECT * FROM health_settings LIMIT 1')
    if (!rows[0]) {
      await db.execute(
        'INSERT INTO health_settings (id, pingIntervalSeconds, timeoutMs, unstableThreshold, offlineTimeoutSeconds, latencyWarnMs, autoRetryCount, escalationMinutes) VALUES (1, ?, ?, ?, ?, ?, ?, ?)',
        [
  ...
  router.put('/health/settings', async (req, res) => {
    ...
    const parsed = healthSettingsSchema.safeParse(req.body)
  ...
  router.get('/summary', async (_req, res) => {
    ...
    const cameraCounts = await dbRows<any>(
      db,
      user?.role === 'ADMIN'
        ? `SELECT
             COUNT(*) AS total,
    ...
    const locationCounts = await dbRows<any>(
      db,
      user?.role === 'ADMIN'
        ...
        : `SELECT COUNT(*) AS locationsActive
           FROM locations l
           INNER JOIN user_location_access ula ON ula.locationId = l.id AND ula.userId = ? AND ula.canView = 1
           WHERE l.active = 1 AND l.deletedAt IS NULL`,
      user?.role === 'ADMIN' ? [] : [user?.id ?? ''],
    )
  ...
  router.post('/locations', async (req, res) => {
    ...
    const dupRows = await dbRows<any>(
      db,
      'SELECT * FROM locations WHERE deletedAt IS NULL AND (UPPER(code) = ? OR LOWER(name) = ?) LIMIT 1',
      [codeKey, nameKey],
    )
    const dup = dupRows[0]
    if (dup) {
  ...
  router.put('/locations/:id', async (req, res) => {
    ...
    const existing = await dbRows<any>(db, 'SELECT * FROM locations WHERE id = ? LIMIT 1', [req.params.id])
    ...
    if (parsed.data.code || parsed.data.name) {
      ...
      const dupRows = await dbRows<any>(
        db,
        'SELECT * FROM locations WHERE id <> ? AND deletedAt IS NULL AND (UPPER(code) = ? OR LOWER(name) = ?) LIMIT 1',
        [req.params.id, nextCode, nextName],
      )
      const dup = dupRows[0]
      if (dup) {
        return res.status(409).json({
    ...
    if (fields.length > 0) {
      const setSql = fields.map((k) => `${k} = ?`).join(', ')
      const values = fields.map((k) => (parsed.data as any)[k])
      await db.execute(`UPDATE locations SET ${setSql} WHERE id = ?`, [...values, req.params.id])
    }

    const updated = (await dbRows<any>(db, 'SELECT * FROM locations WHERE id = ? LIMIT 1', [req.params.id]))[0]
    await logAudit(db, {
      actor,
  ...
  router.get('/cameras', async (_req, res) => {
    ...
    const rows = user?.role === 'ADMIN'
      ? includeDeleted
        ? await dbRows<any>(db, 'SELECT * FROM cameras')
        : await dbRows<any>(
            db,
            'SELECT c.* FROM cameras c INNER JOIN locations l ON l.id = c.locationId WHERE l.deletedAt IS NULL',
          )
      : await dbRows<any>(
          db,
          `SELECT c.*
  ...
  router.put('/cameras/:id', async (req, res) => {
    ...
    const existing = await dbRows<any>(db, 'SELECT * FROM cameras WHERE id = ? LIMIT 1', [req.params.id])
    if (!existing[0]) return res.status(404).json({ error: 'Camera not found' })
    ...
    const updated = (await dbRows<any>(db, 'SELECT * FROM cameras WHERE id = ? LIMIT 1', [req.params.id]))[0]
    await logAudit(db, {
      actor,
  ...
  router.post('/import/cameras', upload.single('file'), async (req: Request, res: Response) => {
    const actor = actorFromReq(req)
    ...
    for (const row of rows) {
      ...
      if (!name || !locationCode || !zone || !ipAddress || !rtspUrl || !vendor) continue

      const locRows = await dbRows<any>(
        db,
        'SELECT * FROM locations WHERE UPPER(code) = ? LIMIT 1',
        [locationCode.toUpperCase()],
      )

      let locId = locRows[0]?.id as string | undefined
      if (!locId) {
        locId = randomUUID()
        const createdAt = new Date().toISOString()
        await db.execute(
          'INSERT INTO locations (id, name, code, active, createdAt) VALUES (?, ?, ?, ?, ?)',
          [locId, locationCode, locationCode, 1, createdAt],
        )
      ...
      await db.execute(
        'INSERT INTO cameras (id, name, locationId, zone, ipAddress, rtspUrl, hlsUrl, cameraType, vendor, installationDate, enabled, status, lastSeenAt, lastLatencyMs, signalStrength) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [
          camId,
          name,
          locId,
  ...
  router.get('/export/cameras.csv', async (_req, res) => {
    ...
    const rows = await dbRows<any>(
      db,
      'SELECT c.*, l.code AS locationCode, l.name AS locationName FROM cameras c LEFT JOIN locations l ON l.id = c.locationId',
    )
  ...
  router.get('/audit', async (_req, res) => {
    ...
    const rows = await dbRows<any>(db, 'SELECT * FROM audit_events ORDER BY at DESC LIMIT 500')
    res.json(
      rows.map((row) => {
