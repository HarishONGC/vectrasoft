...
exports.startStatusSimulator = startStatusSimulator;
const randInt = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
function startStatusSimulator(store, io) {
    ...
    const tick = () => {
        ...
        for (const cam of store.cameras) {
            ...
            const checkPrivateIP = (url) => {
                if (!url)
                    return false;
                return (url.includes('10.205.101.') ||
            ...
            const hasPrivateIP = checkPrivateIP(cam.ipAddress) ||
            ...
            cam.signalStrength = newStatus === 'OFFLINE' ? 0 : (newStatus === 'WARNING' ? randInt(1, 3) : randInt(3, 4));
        }
